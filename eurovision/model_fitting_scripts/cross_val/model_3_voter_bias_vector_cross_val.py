# This script carries out a cross-validation with the 'model_3_voter_bias_vector' model on all of the
# training data (at the time of model development, therefore not including 2023 data).
...
for test_year in [x for x in range(1998,2023)]:
  if test_year == 2020 :
  ...
  az_fit.to_json(f"eurovision/model_output/1998_to_2022_cross_val/model_3_voter_bias_vector_{num_samples}_samples_{num_chains}_chains_test_on_{test_year}.json")
